...
For $X$ to be a (constructive) Kan cubical set, we require to be given
operations $\rup{X} \vec u$ and $\rdo{X} \vec u$, both in $X(I)$, for
every $J,x\subseteq I$.  Here $\vec u$ is an open box with $u_{x0}$
and $u_{x1}$ in $X(I-x)$ in the respective cases $\rup{X} \vec u$ and
$\rdo{X} \vec u$. (From now on we will always tacitly assume that the
open box $\vec u$ is of the right type with respect to
$\rup{X},\rdo{X}$.)  The operations $\rup{X},\rdo{X}$ are to be
thought of as a filling their respective open boxes. Therefore we
require for all $(y,b)\in O(J,x)$:
$$
...
\begin{theorem}
  ...
  condition, then so does $\Gamma\vdash\Pi\,A\,B$.  Moreover,
  $(\Pi\,A\,B) \sigma = \Pi (A \sigma) (B (\sigma \pp, \qq))$ as
  Kan fibrations.
\end{theorem}
...
\begin{proof}
  ...
  $\alpha$ an $I$-cube of $\Gamma$, $x \in I$, and $w$ in $(\Pi\,A\,B)
  \alpha (x = 0)$.  This amounts to define a dependent function $({C
  ...
  \[
  \xymatrix@C=2.5cm{ ({C \alpha}^+_x w)_1 (u_1) \ar[r]^{({C \alpha}^+_x
      w)_{(y)} (u_1 (y))}& ({C \alpha}^+_x w)_1 (u_1) \\
    w_1 u_0 \ar[r]_{w_{(y)} (v (x=0))} & w_1 ({A \alpha}^-_x u_1)
    \ar[u]_\lambda}
  \]
  where the line on the right hand side line is by the defining
  equation~\eqref{eq:3}.  Using the uniform Kan operation of
  $\Gamma.A\vdash B$ for $J=\set{x}$ we define
  \[
  (\rupx{C\alpha}w)_1 (u) = {B(\alpha(y),v)}^-_{y,x}\bigl(\lambda,
  w_{(y)} (v (x=0)), ({C \alpha}^+_x w)_{(y)} (u_1 (y))\bigr)
  \]
