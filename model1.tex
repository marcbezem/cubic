...
already defined $v_i = \tilde\varphi (p (x = i), u_i) \in Y (I-x)$.
Applying $p (x=0)$ to both $v_0$ and $v_1$ gives a path $\bind x
\omega$, where $\omega \in Y (I)$ connecting $v_0$ to $v_1$ along $x$,
and we set $\tilde\varphi (p,u) = \omega$.  Note that the choice of $p
(x = 0) \in (\prop Y) (I-x)$ above is \emph{not} canonical.
