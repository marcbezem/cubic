...
\authorrunning{M. Bezem, T. Coquand and S. Huber}
\Copyright{Marc Bezem, Thierry Coquand and Simon Huber}
...
\serieslogo{}%
\volumeinfo%
       {Ralph Matthes and Aleksy Schubert}%
       {2}%
...
(TYPES~2013)}%
     ...
     \EventShortName{TYPES 2013}
     \DOI{10.4230/LIPIcs.TYPES.2013.XXX}% XXX is not the final value!
...
% (One other possible attempt to solve this problem is by marking the
% degenerate elements, but it is not clear yet in this approach how to
% treat exponentials in a satisfactory way.)
...
implemented a type checker\footnote{Available at:
  \url{http://github.com/simhu/cubical}} based on this paper.  This
...
names.  
\begin{definition}\label{catC}
The category $\CC$ of names and substitutions has as
objects all finite decidable subsets of the name space, 
denoted by $I,J,K,\dots$.  A morphism $f:I\to J$ is
a map $I \to J\cup \set{0,1}$ such that $f(i) = f(j)$ if{f} $i=j$
whenever $f(i)$ and $f(j)$ are in $J$. 
\end{definition}
Notice that $\set{0,1}$ is
...
\begin{definition}\label{cubicalset}
A \emph{cubical set} is a covariant functor $\CC\to{\sf Set}$.
\end{definition}
Let $X$ be a
...
% $(x=x'): I,x \to I,x'$ and $(x'=x): I,x' \to I,x$ renaming only $x$
% to $x'$ and vice versa are each other's inverse, and we can identify
% $\omega\in X(x_1,\dots,x_n,x)$ with $\omega'\in X(x_1,\dots,x_n,x')$
% whenever $\omega' = \omega (x=x')$, or, equivalently, $\omega =
% \omega' (x'=x)$.  Due to the isomorphisms $x=x'$ and $x'=x$, the
% name of the bound variable does not matter (as long as it is not in
% $I$).
...
this Agda development\footnote{Available at:
  \url{http://www.cse.chalmers.se/~nad/}} is accompanying the
paper~\cite{CoquandDanielsson}.

Let us define the more common elimination operator of C.\
Paulin-Mohring~\cite{Moh93} from the above operations---with the difference that
...
well.\footnote{See the implementation at
  \url{http://github.com/simhu/cubical}.}
...
\newblock{Preprint, 2012, \url{http://arxiv.org/abs/1211.2851}.}
