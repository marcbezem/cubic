...
$\vec u f$ for the family indexed by $O (f (J), f(x))$ with components
$(\vec u f)_{(f y)b} = u_{yb} (f-y)$ in $X (K - f(y))$.
...
distinguish cases in which $a$ is degenerated or not.  One can show
...
\set{0,1,x,y}$ ($\interval$ has four degenerated squares, see the
display below); and so on.  The square $x$ varies in direction $x$,
but is constant in direction $y$, and hence degenerated.  Similarly
...
non-degenerated in two directions:
\[
...
other point. In higher dimensions all arguments are degenerated,
...
= \bind{x} \app ((\varphi \rho)_1, \omega)$ for $p \rho = \bind x
\omega$.  This satisfies the equations
\begin{align*}
...
Our cubical sets do not have \emph{connections}, like those in
\cite{Williamson}.  In \cite[III.4.2.10]{Williamson}, Williamson
raises the question of a straightforward geometric argument replacing
his footnote~19 to exhibit the group structure of $\pi_n(X,x)$ for Kan
cubical sets \emph{without} connections.  Adapting his geometric
argument, we can answer his question positively for our Kan cubical
sets.  The argument below also works for cubical sets as presented
in~\cite{Kan}.
