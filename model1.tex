...
\to \Gamma.A$ by $(\sigma, u) \beta = (\sigma \beta, \sigma u)$.  This
...
for $\Gamma \vdash A$ and $\Gamma.A \vdash B$ is defined to be by sums
in each stage, i.e., for an $I$-cube $\alpha$ in $\Gamma$,
...
sections given by first and second projection give interpretations for
...
We have to define $w=\rup{(\Pi~A~B)\alpha} w_0 \in (\Pi~A~B)\alpha$ for all $w_0 \in (\Pi~A~B)\alpha(x=0)$.
...
$$
v = \rdoxy{A\alpha(y)}(u_1,u,\rdo{A\alpha} u_1),~~~\text{where the first $u_1$ is $u_1(y) \in A\alpha(y)(x=1)$}
$$
The line $v(x=0)$
